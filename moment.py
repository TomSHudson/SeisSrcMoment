...
def time_to_freq_domain_unnormallised(tr_data, sampling_rate):
    ...
    freq = freqTemp[0:int(lengthTrace/2)] # Positive frequency only 
    spectrumTraceTemp = fft(tr_data)/lengthTrace # Perform FFT on trace to find frequency component
    spectrumTrace = spectrumTraceTemp[0:int(lengthTrace/2)] # Only take positive half of spectrum
...
def rotate_ZNE_to_LQT_axes(st, NLLoc_event_hyp_filename, stations_not_to_process=[]):
    ...
    for i in np.arange(len(list(arrival_times_dict['station_arrival_times'].keys()))):
        # Get station name and station lat and lon:
        statName = list(arrival_times_dict['station_arrival_times'].keys())[i]
...
def get_P_arrival_time_at_station_from_NLLoc_event_hyp_file(NLLoc_event_hyp_filename, station):
    ...
    var = subprocess.check_output('grep "'+station+'" '+NLLoc_event_hyp_filename+' | grep "? P" | '+"awk '{print $7$8$9}'", shell=True).decode("utf-8") 
...
def get_theta_and_phi_for_stations_from_NLLoc_event_hyp_file(NLLoc_event_hyp_filename, station):
    """Function to get theta and phi, in terms of lower hemisphere plot, for calculating radiation pattern components in other functions. Takes in NLLoc hyp file and station name, and returns theta and phi spherical coords."""
    # Get theta and phi in terms of lower hemisphere plot (as can get radiation pattern from lower or upper)
    var = subprocess.check_output('grep "'+station+'" '+NLLoc_event_hyp_filename+' | grep "? P" | '+"awk '{print $23,$25}'", shell=True).decode("utf-8") 
...
def get_event_hypocentre_station_distance(NLLoc_event_hyp_filename, station):
    ...
    var = subprocess.check_output('grep "HYPOCENTER" '+NLLoc_event_hyp_filename+" | awk '{print $3,$5,$7}'", shell=True).decode("utf-8") 
    event_x = float(var.split(" ")[0])
    event_y = float(var.split(" ")[1])
    event_z = float(var.split(" ")[2])
    # Get station coords (in km grid):
    var = subprocess.check_output('grep "'+station+'" '+NLLoc_event_hyp_filename+' | grep "? P" | '+"awk '{print $19,$20,$21}'", shell=True).decode("utf-8") 
...
def calc_moment(mseed_filename, instruments_gain_filename, NLLoc_event_hyp_filename, stations_to_calculate_moment_for, density, Vp, Q=150, stations_not_to_process=[], MT_data_filename=None, MT_six_tensor=[], verbosity_level=0):
    ...
    instruments_gain_filename - Filenmae of a text file containing the instrument gains, with the columns: [Station instrument_gain(Z N E) digitaliser_gain(Z N E)]. (str)
    ...
    Vp - The P wave velocity of the medium (float)
    Optional:
    ...
    if MT_data_filename:
        ...
        print("Error: Need to specify MT_six_tensor or MT_data_filename Exiting.")
        sys.exit()
...
if __name__ == "__main__":
    ...
    # Get seismic moment:
    seis_moment = calc_moment(mseed_filename, instruments_gain_filename, NLLoc_event_hyp_filename, stations_to_calculate_moment_for, density, Vp, Q=150, stations_not_to_process=stations_not_to_process, MT_six_tensor=MT_six_tensor, verbosity_level=verbosity_level)
